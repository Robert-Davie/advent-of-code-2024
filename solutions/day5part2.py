...
def swap(given_list, rule_parts):
    for i in range(len(given_list)):
        if given_list[i] == rule_parts[0]:
            given_list[i] = -1
    for i in range(len(given_list)):
        if given_list[i] == rule_parts[1]:
            given_list[i] = rule[0]
    for i in range(len(given_list)):
        if given_list[i] == -1:
            given_list[i] = rule_parts[1]


sorted_list = []
for this_list in sorting_list:
    is_sorted = False
    while not is_sorted:
        is_sorted = True
        for rule in rules:
            if check_rule_broken(this_list, rule):
                swap(this_list, rule)
                is_sorted = False
    sorted_list.append(this_list)
